...
async function sbGet(table, id) {
  try {
    const r = await fetch(SB(table + "?id=eq." + id), { headers });
...
async function sbGetAll(table) {
  try {
    const r = await fetch(SB(table + "?order=created_at.desc"), { headers });
    const d = await r.json();
...
async function sbGetAllProducts() {
  try {
    const r = await fetch(SB("products?order=created_at.asc"), { headers });
    const d = await r.json();
    return Array.isArray(d) ? d.map(row => row.data) : [];
  } catch { return []; }
}
async function sbInsertProduct(prod) {
  try {
    await fetch(SB("products"), {
      method: "POST", headers,
      body: JSON.stringify({ id: prod.id, data: prod })
    });
  } catch {}
}
...
async function sbDeleteProduct(id) {
  try {
    await fetch(SB("products?id=eq." + id), { method: "DELETE", headers });
  } catch {}
}
...
const defaultSettings = {
  shopName: "MEGHDOOT",
  shopTagline: "Mills Authorised Stockists of RAYMOND SUITINGS.",
  shopAddress: "BARA BAZAR, B. DEOGHAR-814112 (JHARKHAND)",
  shopPhone: "9876543210",
  gstin: "20ACDPJ3255B1ZG",
  stateCode: "20",
  footerNote: "No Guaranty of colours in the fashion of world.",
  signoff: "For MEGHDOOT",
  gstLow: 5,
  gstHigh: 18,
  gstThreshold: 2500,
  currency: "₹",
...
function LoginScreen({ onLogin, settings }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f3f4f6", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, sans-serif" }}>
      ...
      <div style={{ fontWeight: 900, fontSize: 24, color: "#1e3a5f", marginBottom: 4 }}>FabricBill</div>
      <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 32 }}>Select your role to continue</div>
      {!role ? (
...
function InvoiceView({ txn, settings, onClose }) {
  ...
  const doPrint = () => {
    ...
    win.document.write(`<html><head><title>Invoice ${txn.invoiceNo}</title>
      ...
      <button onclick="window.print();window.close();"
        style="padding:10px 24px;background:#1e3a5f;color:#fff;border:none;border-radius:8px;font-size:14px;cursor:pointer;">
        Print / Save PDF</button></body></html>`);
    win.document.close();
  };
  ...
  const doSharePDF = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        a.href = url; a.download = "Invoice-" + txn.invoiceNo.replace("/", "-") + ".pdf";
        a.click(); URL.revokeObjectURL(url);
        alert("PDF downloaded! You can now share it manually on WhatsApp.");
      }
    } catch (e) {
      alert("Could not generate PDF. Please use Print/PDF option instead.");
    }
  };

  const doWhatsApp = () => {
    const f2 = n => fmt(n, settings.currency);
    const halfRate2 = (blRate / 2).toFixed(1);
    let msg = "🧵 *" + settings.shopName + "*\n";
    msg += "_" + (settings.shopTagline || "") + "_\n";
    msg += settings.shopAddress + "\n";
    msg += "GSTIN: " + settings.gstin + "\n";
    msg += "─────────────────────\n";
    msg += "🧾 *INVOICE: " + txn.invoiceNo + "*\n";
    msg += "📅 Date: " + new Date(txn.date).toLocaleDateString("en-IN") + "\n";
    msg += "👤 Buyer: *" + txn.customer.name + "*\n";
    if (txn.customer.phone) msg += "📞 Ph: " + txn.customer.phone + "\n";
    msg += "─────────────────────\n";
    msg += "*Items:*\n";
    txn.items.forEach((item, i) => {
      msg += (i + 1) + ". " + item.name + "\n";
      msg += "   " + item.qty + " x " + f2(item.price) + " = *" + f2(item.price * item.qty) + "* (GST " + (item.gstRate * 100).toFixed(0) + "%)\n";
    });
    msg += "─────────────────────\n";
    msg += "Gross Total: " + f2(txn.subtotal) + "\n";
    if (txn.discount > 0) msg += "Discount: -" + f2(txn.discount) + "\n";
    msg += "Taxable Value: " + f2(txn.taxable) + "\n";
    msg += "CGST @" + halfRate2 + "%: " + f2(cgst) + "\n";
    msg += "SGST @" + halfRate2 + "%: " + f2(sgst) + "\n";
    if (txn.roundOff && txn.roundOff !== 0) msg += "Round Off: " + (txn.roundOff > 0 ? "+" : "") + f2(txn.roundOff) + "\n";
    msg += "─────────────────────\n";
    msg += "💰 *Net Amount: " + f2(Math.round(txn.total)) + "*\n";
    msg += "💳 Payment: " + txn.paymentMode + "\n";
    msg += "─────────────────────\n";
    msg += "_" + amtWords + "_\n\n";
    msg += settings.footerNote + "\n";
    msg += "*" + settings.signoff + "*";
    const encoded = encodeURIComponent(msg);
    if (txn.customer.phone) {
      window.open("https://wa.me/91" + txn.customer.phone + "?text=" + encoded, "_blank");
    } else {
  ...
  const doThermal = () => {
    ...
    txn.items.forEach(item => {
      const desc = item.name + " (" + item.qty + "x" + item.price.toFixed(2) + ")", amt = (item.price * item.qty).toFixed(2);
      t += row(desc.slice(0, W - amt.length - 1), amt) + "\n  GST: " + (item.gstRate * 100).toFixed(0) + "%\n";
    });
    t += line + "\n";
      ...
      <div style={{ background: "#fff", borderRadius: "16px 16px 0 0", padding: 16, width: "100%", maxWidth: 480, margin: "0 auto", maxHeight: "92vh", overflowY: "auto" }}>
        <div id="inv-print" style={{ border: "2px solid #000", padding: 10, fontFamily: "monospace", fontSize: 11 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            ...
            <span><b>Date:</b> {new Date(txn.date).toLocaleDateString("en-IN")}</span>
          </div>
          ...
          <div style={{ display: "flex", borderLeft: BDR, borderRight: BDR, borderBottom: BDR }}>
            ...
            <div style={{ width: 200 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 6px", fontWeight: 900, fontSize: 12 }}>
                <span>Net Value</span><span>{f(netDisplay)}</span>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  const saveSettings = useCallback(async (d) => {
    ...
    await sbUpsert("settings", "main", d); setSyncStatus("ok");
  }, []);

  const addProduct = useCallback(async (prod) => {
    const newProd = { ...prod, id: genId() };
    setProducts(p => [...p, newProd]);
    setSyncStatus("syncing");
    await sbInsertProduct(newProd);
    setSyncStatus("ok");
    return newProd;
  }, []);

  const updateProduct = useCallback(async (updatedProd) => {
    setProducts(p => p.map(pr => pr.id === updatedProd.id ? updatedProd : pr));
    setSyncStatus("syncing");
    await sbUpdateProduct(updatedProd);
    setSyncStatus("ok");
  }, []);
  ...
  const saveCustomers = useCallback(async (d) => {
    ...
    await Promise.all(d.map(c => sbUpsert("customers", c.id, c)));
    setSyncStatus("ok");
  }, []);

  const addTransaction = useCallback(async (txn) => {
    const updated = [txn, ...transactions];
    setTransactions(updated); setSyncStatus("syncing");
    await sbInsert("transactions", txn); setSyncStatus("ok");
  }, [transactions]);

  const deleteTransaction = useCallback(async (id) => {
    const updated = transactions.filter(t => t.id !== id);
    setTransactions(updated); setSyncStatus("syncing");
    await sbDelete("transactions", id); setSyncStatus("ok");
  }, [transactions]);
  ...
  const collectedTaxable = useCollected ? Math.round(collected / (1 + blendedRate) * 100) / 100 : Math.round((grandSubtotal - manualDiscount) * 100) / 100;
  ...
  const roundOff = Math.round((netRounded - netBeforeRound) * 100) / 100;
  const netAmount = netRounded;
  ...
  const handlePayment = async () => {
    ...
    const txn = {
      id: genId(), invoiceNo: genInvoiceNo(transactions),
      date: new Date().toLocaleString(), customer: cust, items: validCart,
      subtotal: grandSubtotal, discount: collectedDiscount, taxable: collectedTaxable,
      gst: collectedGST, roundOff, total: netAmount, paymentMode, settings: { ...settings }
    };
  ...
  const dayTotals = (txns) => ({
    gross: txns.reduce((s, t) => s + t.subtotal, 0),
    discount: txns.reduce((s, t) => s + t.discount, 0),
    taxable: txns.reduce((s, t) => s + t.taxable, 0),
    gst: txns.reduce((s, t) => s + t.gst, 0),
  ...
  if (!ready) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", flexDirection: "column", gap: 12, background: "#f3f4f6" }}>
      ...
      <div style={{ color: "#9ca3af", fontSize: 13 }}>Connecting to cloud...</div>
    </div>
  );

  if (!role) return <LoginScreen onLogin={r => { setRole(r); setTab("billing"); }} settings={settings} />;
    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: "#f3f4f6", minHeight: "100vh", maxWidth: 480, margin: "0 auto", paddingBottom: 80 }}>
      <div style={{ background: "#1e3a5f", color: "#fff", padding: "14px 16px", position: "sticky", top: 0, zIndex: 50, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 11, opacity: 0.7, marginTop: 2 }}>{new Date().toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", year: "numeric" })}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button onClick={() => { setRole(null); setTab("billing"); }} style={{ background: "rgba(255,255,255,0.15)", border: "none", borderRadius: 8, color: "#fff", padding: "5px 10px", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>Logout</button>
        </div>
      </div>
                  ...
                  <div key={item.uid} style={{ background: "#f9fafb", borderRadius: 10, padding: 12, marginBottom: 10, border: "1px solid #e5e7eb" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <input type="number" min={0} value={item.price} onChange={e => updateLine(item.uid, "price", e.target.value)} style={{ ...inp, padding: "8px 10px", fontSize: 13 }} />
                      </div>
                      <div style={{ width: 110 }}>
            ...
            {cart.length > 0 && (
              <div style={card}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, marginBottom: 8 }}>
                  <span style={{ color: "#6b7280" }}>Subtotal</span><span style={{ fontWeight: 600 }}>{f(grandSubtotal)}</span>
                </div>
                  ...
                  <div key={txn.id} style={{ borderBottom: "1px solid #f3f4f6", paddingBottom: 12, marginBottom: 12 }}>
                    ...
                    {txn.items.map(item => (
                      <div key={item.uid} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6b7280", marginTop: 3 }}>
                        <span>{item.name} × {item.qty} @ {f(item.price)}</span>
                        <span>{(item.gstRate * 100).toFixed(0)}% GST</span>
                      </div>
                    ))}
                  ...
                  return (
                    <>
                      ...
                      {filteredDays.length === 0
                        ? <div style={{ color: "#9ca3af", textAlign: "center", padding: "24px 0" }}>No transactions found</div>
        ...
        {tab === "products" && isAdmin && (
          <>
            ...
            <div style={card}>
              ...
              {products.length === 0 && <div style={{ color: "#9ca3af", fontSize: 13, textAlign: "center", padding: "16px 0" }}>No products yet. Add your first product above.</div>}
              {products.map((p, i) => (
