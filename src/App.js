...
async function sbGet(table, shopCode, id) {
  try {
    ...
    const d = await r.json(); return d && d.length > 0 ? d[0].data : null;
  } catch { return null; }
}
async function sbUpsert(table, shopCode, id, data) {
  try {
    await fetch(SB(table), { method: "POST", headers: { ...hdrs, "Prefer": "resolution=merge-duplicates,return=representation" }, body: JSON.stringify({ id: scopedId(shopCode, id), data }) });
  } catch {}
}
async function sbGetAll(table, shopCode) {
  try {
    ...
    const d = await r.json(); return Array.isArray(d) ? d.map(row => row.data) : [];
  } catch { return []; }
}
async function sbInsert(table, shopCode, data) {
  try { await fetch(SB(table), { method: "POST", headers: hdrs, body: JSON.stringify({ id: scopedId(shopCode, data.id), data }) }); } catch {}
}
async function sbUpdate(table, shopCode, id, data) {
  try {
    await fetch(SB(table), { method: "POST", headers: { ...hdrs, "Prefer": "resolution=merge-duplicates,return=representation" }, body: JSON.stringify({ id: scopedId(shopCode, id), data }) });
  } catch {}
}
async function sbDelete(table, shopCode, id) {
  try { await fetch(SB(table) + "?id=eq." + encodeURIComponent(scopedId(shopCode, id)), { method: "DELETE", headers: hdrs }); } catch {}
}
...
async function sbInsertProduct(shopCode, prod) {
  try { await fetch(SB("products"), { method: "POST", headers: hdrs, body: JSON.stringify({ id: scopedId(shopCode, prod.id), data: prod }) }); } catch {}
}
async function sbUpdateProduct(shopCode, prod) {
  try { await fetch(SB("products"), { method: "POST", headers: { ...hdrs, "Prefer": "resolution=merge-duplicates,return=representation" }, body: JSON.stringify({ id: scopedId(shopCode, prod.id), data: prod }) }); } catch {}
}
async function sbDeleteProduct(shopCode, id) {
  try { await fetch(SB("products") + "?id=eq." + encodeURIComponent(scopedId(shopCode, id)), { method: "DELETE", headers: hdrs }); } catch {}
}

const PAYMENT_MODES = ["Cash", "UPI", "Card", "Credit"];
const GST_RATES = [0, 5, 18]; // removed 12 and 28
...
function ShopCodeScreen({ onEnter }) {
  ...
  const registerShop = async () => {
    ...
    } catch { setRegistering(false); setErr("Registration failed. Check internet."); }
  };
  ...
  if (screen === "register") return (
    <div style={{ minHeight:"100vh", background:"#f3f4f6", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"-apple-system,sans-serif", padding:24 }}>
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:28, boxShadow:"0 4px 24px rgba(0,0,0,0.1)", width:"100%", maxWidth:340 }}>
        ...
        <div style={{ background:"#f0fdf4", borderRadius:8, padding:"8px 12px", fontSize:11, color:"#166534", marginBottom:16 }}>ℹ️ Default Staff PIN is <b>0000</b>. Change later in Settings.</div>
...
function InvoiceView({ txn, settings, onClose }) {
  ...
  const doWhatsApp = () => {
    ...
    msg += "─────────────────────\n_"+amtWords+"_\n\n"+settings.footerNote+"\n*"+settings.signoff+"*";
    if (txn.cancelled) msg = "❌ *CANCELLED*\n\n" + msg;
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"flex-end", zIndex:100 }}>
      <div style={{ background:"#fff", borderRadius:"16px 16px 0 0", padding:16, width:"100%", maxWidth:480, margin:"0 auto", maxHeight:"92vh", overflowY:"auto" }}>
        {txn.cancelled && <div style={{ background:"#fee2e2", borderRadius:8, padding:"10px 14px", marginBottom:12, fontWeight:800, color:"#dc2626", fontSize:15, textAlign:"center" }}>❌ THIS INVOICE IS CANCELLED</div>}
        <div id="inv-print" style={{ border:"2px solid #000", padding:10, fontFamily:"monospace", fontSize:11, opacity:txn.cancelled?0.5:1 }}>
          <div style={{ textAlign:"center", borderBottom:BDR, paddingBottom:6, marginBottom:6 }}>
            {txn.cancelled && <div style={{ fontWeight:900, fontSize:14, color:"#dc2626", marginBottom:4 }}>*** CANCELLED ***</div>}
          ...
          <div style={{ display:"flex", borderLeft:BDR, borderRight:BDR, borderBottom:BDR }}>
            <div style={{ flex:1, padding:"6px 8px", borderRight:BDR, fontSize:10 }}>
              ...
              <div style={{ marginTop:6, paddingTop:4, borderTop:"1px dashed #999" }}>
                ...
                {creditAmt > 0 && <div style={{ fontWeight:700, color:"#dc2626", marginTop:2 }}>⚠ Amount Due: {f(creditAmt)}</div>}
              </div>
            </div>
            <div style={{ width:200 }}>
...
// ── Edit Invoice Modal ────────────────────────────────────────────────────
function EditInvoiceModal({ txn, products, settings, onSave, onCancel, onCancelInvoice }) {
  ...
  const taxable = Math.round((grandSubtotal-Math.min(discount,grandSubtotal))*100)/100;
  ...
  const handleSave = () => {
    const validItems = cartWithTax.filter(i=>i.name&&i.price>0&&i.qty>0);
    if (validItems.length===0) { alert("Add at least one item."); return; }
    const updatedTxn = { ...txn, items:validItems, subtotal:grandSubtotal, discount:Math.min(discount,grandSubtotal), taxable, gst, roundOff:0, total:netAmount, payments, paymentMode:payments[0]?.mode||"Cash", editedAt:new Date().toLocaleString() };
    onSave(updatedTxn);
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:200, display:"flex", alignItems:"flex-end" }}>
      <div style={{ background:"#fff", borderRadius:"16px 16px 0 0", padding:16, width:"100%", maxWidth:480, margin:"0 auto", maxHeight:"92vh", overflowY:"auto" }}>
        ...
        <div style={{ fontSize:11, color:"#9ca3af", marginBottom:14 }}>Changes allowed within 24 hours of creation</div>
        ...
        {payments.map((pm,idx)=>(
          <div key={idx} style={{ display:"flex", gap:6, marginBottom:8, alignItems:"center" }}>
            ...
            <input type="number" value={pm.amount} onChange={e=>updatePayment(idx,"amount",parseFloat(e.target.value)||0)} style={{ width:100, padding:"9px 10px", border:"1px solid #d1d5db", borderRadius:8, fontSize:13 }} />
        ...
        {payments.length < 4 && <button onClick={addPayment} style={{ width:"100%", padding:"8px 0", background:"#f3f4f6", color:"#374151", border:"1px dashed #d1d5db", borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer", marginBottom:8 }}>+ Add Payment Mode</button>}
        {Math.abs(totalPaid-netAmount)>1 && <div style={{ color:"#dc2626", fontSize:12, marginBottom:8 }}>⚠ Total paid ({fmt(totalPaid,settings.currency)}) ≠ Net ({fmt(netAmount,settings.currency)})</div>}
        ...
        <button onClick={()=>{ if(window.confirm("Cancel this invoice? This cannot be undone.")) onCancelInvoice(); }} style={{ width:"100%", marginTop:8, padding:"11px 0", background:"#fee2e2", color:"#dc2626", border:"1px solid #fca5a5", borderRadius:10, fontSize:13, fontWeight:800, cursor:"pointer" }}>🚫 Cancel Invoice</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [role, setRole] = useState(()=>{
    try {
      ...
      localStorage.removeItem("fabricbill_session"); return null;
    } catch { return null; }
  });
  ...
  const handleEnterShop = code => {
    ...
    setShopCode(code); setReady(false);
  };
  ...
  const addProduct = useCallback(async prod => {
    ...
    await sbInsertProduct(shopCode, np); setSyncStatus("ok"); return np;
  }, [shopCode]);
  ...
  const updateTransaction = useCallback(async txn => {
    const upd = transactions.map(t=>t.id===txn.id?txn:t);
    setTransactions(upd); setSyncStatus("syncing");
    await sbUpdate("transactions", shopCode, txn.id, txn); setSyncStatus("ok");
  }, [transactions, shopCode]);
  ...
  // GST based on taxable value after discount
  const getGstRate = (name, taxableValue) => {
    const prod = products.find(p=>p.name.toLowerCase()===name.toLowerCase());
    if (prod && prod.gstOverride!==null && prod.gstOverride!==undefined) return prod.gstOverride/100;
    return taxableValue >= settings.gstThreshold ? settings.gstHigh/100 : settings.gstLow/100;
  };

  const grandSubtotal = cart.reduce((s,item)=>s+(parseFloat(item.price)||0)*(parseFloat(item.qty)||0),0);
  ...
  const effectiveTaxable = useCollected
    ? Math.floor(collected/(1+(grandSubtotal>0?cart.reduce((s,i)=>{const p=parseFloat(i.price)||0,q=parseFloat(i.qty)||0,sub=p*q; return s+(sub/grandSubtotal)*getGstRate(i.name,Math.max(0,grandSubtotal-manualDiscount));},0):0))*100)/100
    : Math.round((grandSubtotal-manualDiscount)*100)/100;

  const cartWithTax = cart.map(item=>{
    const price=parseFloat(item.price)||0, qty=parseFloat(item.qty)||0, subtotal=price*qty;
    const rate = getGstRate(item.name, effectiveTaxable);
    return { ...item, price, qty, subtotal, gstRate:rate, total:subtotal*(1+rate) };
  });
  const blendedRate = grandSubtotal>0 ? cartWithTax.reduce((s,i)=>s+(i.subtotal/grandSubtotal)*i.gstRate,0) : 0;
  ...
  const addPaymentRow = () => { if (payments.length<4) setPayments(p=>[...p,{ mode:"Cash", amount:"" }]); };
  const removePaymentRow = idx => setPayments(p=>p.filter((_,i)=>i!==idx));

  // Voice handler — parses "item name price quantity"
  const handleVoiceResult = (transcript, uid) => {
    const t = transcript.toLowerCase().trim();
    // Try to extract: last number = qty, second last = price, rest = name
    const nums = t.match(/\d+(\.\d+)?/g);
    if (nums && nums.length >= 2) {
      const qty = nums[nums.length-1];
      const price = nums[nums.length-2];
      const name = t.replace(/\d+(\.\d+)?/g,"").replace(/\s+/g," ").trim();
      updateLine(uid, "name", name || t);
      updateLine(uid, "price", price);
      updateLine(uid, "qty", qty);
    } else if (nums && nums.length===1) {
      const name = t.replace(/\d+(\.\d+)?/g,"").trim();
      updateLine(uid, "name", name||t);
      updateLine(uid, "price", nums[0]);
    } else {
      updateLine(uid, "name", t);
    }
  };

  const handlePayment = async () => {
    if (validCart.length===0) return;
    if (Math.abs(totalPayments-netAmount)>1) { alert("Payment total doesn't match net amount. Please adjust."); return; }
    const cust = customers.find(c=>c.id===selectedCustomer)||customers[0];
    const txn = {
      id:genId(), invoiceNo:genInvoiceNo(transactions), date:new Date().toLocaleString(),
      customer:cust, items:validCart, subtotal:grandSubtotal, discount:collectedDiscount,
      taxable:collectedTaxable, gst:collectedGST, roundOff, total:netAmount,
      payments, paymentMode:payments[0]?.mode||"Cash", settings:{...settings}
    };
  ...
  const handleSaveSettings = async () => {
    ...
    setPayments([{ mode:draftSettings.defaultPaymentMode, amount:"" }]);
  ...
  const handleEditSave = async txn => {
    await updateTransaction(txn); setEditTxn(null); setShowReceipt(txn);
  };
  const handleCancelInvoice = async txn => {
    const cancelled = { ...txn, cancelled:true, cancelledAt:new Date().toLocaleString() };
    await updateTransaction(cancelled); setEditTxn(null); setShowReceipt(cancelled);
  };
  ...
  const dayTotals = txns => ({
    ...
    net:txns.filter(t=>!t.cancelled).reduce((s,t)=>s+t.total,0), count:txns.length,
    cash:txns.filter(t=>!t.cancelled).reduce((s,t)=>s+(t.payments?t.payments.find(p=>p.mode==="Cash")?.amount||0:(t.paymentMode==="Cash"?t.total:0)),0),
    upi:txns.filter(t=>!t.cancelled).reduce((s,t)=>s+(t.payments?t.payments.find(p=>p.mode==="UPI")?.amount||0:(t.paymentMode==="UPI"?t.total:0)),0),
    card:txns.filter(t=>!t.cancelled).reduce((s,t)=>s+(t.payments?t.payments.find(p=>p.mode==="Card")?.amount||0:(t.paymentMode==="Card"?t.total:0)),0),
    credit:txns.filter(t=>!t.cancelled).reduce((s,t)=>s+(t.payments?t.payments.find(p=>p.mode==="Credit")?.amount||0:(t.paymentMode==="Credit"?t.total:0)),0),
  });
  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background:"#f3f4f6", minHeight:"100vh", maxWidth:480, margin:"0 auto", paddingBottom:80 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                ...
                {settings.enableVoice && (
                  <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                    ...
                    <span style={{ fontSize:10, color:"#9ca3af" }}>🎤 Voice</span>
                  </div>
                )}
              </div>
              {cart.map(item=>{
                const p=parseFloat(item.price)||0, q=parseFloat(item.qty)||0;
                const rate=getGstRate(item.name, effectiveTaxable), lineTotal=p*q*(1+rate);
            ...
            {cart.length>0 && (
              <div style={card}>
                ...
                <div style={{ marginTop:14 }}>
                  ...
                  {payments.length<4 && <button onClick={addPaymentRow} style={{ width:"100%", padding:"8px 0", background:"#f3f4f6", color:"#374151", border:"1px dashed #d1d5db", borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer", marginBottom:4 }}>+ Split Payment</button>}
                  {payments.length>1 && Math.abs(totalPayments-netAmount)>1 && <div style={{ color:"#dc2626", fontSize:12, marginTop:4 }}>⚠ Total paid {f(totalPayments)} ≠ Net {f(netAmount)}</div>}
                  {creditAmount>0 && <div style={{ background:"#fee2e2", borderRadius:8, padding:"8px 12px", marginTop:8, fontWeight:700, color:"#dc2626", fontSize:13 }}>⚠ Amount Due (Credit): {f(creditAmount)}</div>}
                </div>
        ...
        {tab==="customers" && (
          <>
            ...
            <div style={card}>
              ...
              {customers.map((c,i)=>{
                const txns=transactions.filter(t=>t.customer.id===c.id), spent=txns.filter(t=>!t.cancelled).reduce((s,t)=>s+t.total,0);
                const creditDue=txns.filter(t=>!t.cancelled).reduce((s,t)=>s+(t.payments?t.payments.find(p=>p.mode==="Credit")?.amount||0:(t.paymentMode==="Credit"?t.total:0)),0);
                return (
                  <div key={c.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:i<customers.length-1?"1px solid #f3f4f6":"none" }}>
                    <div>
                      ...
                      {creditDue>0 && <div style={{ fontSize:11, color:"#dc2626", fontWeight:700 }}>⚠ Credit Due: {f(creditDue)}</div>}
                    </div>
          ...
          <>
            ...
            {(histView==="bills"||!isAdmin) && (
              <div style={card}>
                ...
                : transactions.map(txn=>{
                  const canEdit = isAdmin && !txn.cancelled && isWithin24Hours(txn.date);
                  ...
                  const txnCredit = txn.payments ? txn.payments.find(p=>p.mode==="Credit")?.amount||0 : (txn.paymentMode==="Credit"?txn.total:0);
                  return (
                    <div key={txn.id} style={{ borderBottom:"1px solid #f3f4f6", paddingBottom:12, marginBottom:12, opacity:txn.cancelled?0.5:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <div>
                          <div style={{ fontWeight:700, fontSize:14 }}>{txn.customer.name} {txn.cancelled && <span style={{ color:"#dc2626", fontSize:11 }}>❌ CANCELLED</span>}</div>
                        ...
                        <div style={{ textAlign:"right", display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
                          <div style={{ fontWeight:800, fontSize:15, color:txn.cancelled?"#9ca3af":"#16a34a" }}>{f(txn.total)}</div>
                          <div style={{ fontSize:11, color:"#9ca3af" }}>{pmtLabel}</div>
                          {txnCredit>0 && <div style={{ fontSize:11, color:"#dc2626", fontWeight:700 }}>Due: {f(txnCredit)}</div>}
                          <div style={{ display:"flex", gap:4 }}>
                            ...
                            {isAdmin && <button onClick={()=>{ if(window.confirm("Delete this bill?")) deleteTransaction(txn.id); }} style={{ background:"#fee2e2", border:"none", borderRadius:4, color:"#dc2626", padding:"2px 8px", fontSize:10, cursor:"pointer", fontWeight:700 }}>🗑</button>}
                          </div>
                        </div>
                      </div>
                      {txn.items.map(item=>(
            ...
            {isAdmin && histView==="report" && !selectedDay && (
              <div style={card}>
                ...
                {(()=>{
                  const filteredDays = days.filter(day=>{ const d=new Date(day); if(reportFrom&&d<new Date(reportFrom))return false; if(reportTo&&d>new Date(reportTo+"T23:59:59"))return false; return true; });
                  ...
                  return (
                    <>
                      ...
                      : filteredDays.map((day,i)=>{ const t=dayTotals(grouped[day]); return (
            ...
            {isAdmin && histView==="report" && selectedDay && (()=>{ const t=dayTotals(grouped[selectedDay]); return (
              <div>
                ...
                <div style={card}>
                  ...
                  {grouped[selectedDay].map(txn=>(
                    <div key={txn.id} style={{ borderBottom:"1px solid #f3f4f6", paddingBottom:10, marginBottom:10, display:"flex", justifyContent:"space-between", opacity:txn.cancelled?0.5:1 }}>
                      <div><div style={{ fontWeight:700, fontSize:13 }}>{txn.customer.name} {txn.cancelled&&<span style={{ color:"#dc2626",fontSize:10 }}>❌</span>}</div><div style={{ fontSize:11, color:"#9ca3af" }}>{txn.invoiceNo} · {txn.payments?txn.payments.filter(p=>p.amount>0).map(p=>p.mode).join("+"):txn.paymentMode}</div></div>
                      <div style={{ fontWeight:800, color:txn.cancelled?"#9ca3af":"#16a34a", fontSize:14 }}>{f(txn.total)}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {tab==="products" && isAdmin && (
          <>
            <div style={card}>
              ...
              <div style={{ fontSize:12, color:"#9ca3af", marginBottom:12 }}>★ = Fixed GST override · Default = threshold-based</div>
        ...
        {tab==="settings" && isAdmin && (
          <>
            ...
            <div style={card}>
              ...
              {draftSettings.enableVoice && <div style={{ marginTop:8, background:"#eff6ff", borderRadius:8, padding:"8px 12px", fontSize:12, color:"#1e40af" }}>Voice works on Chrome browser. Say: "item name price quantity" e.g. "silk saree 1500 2"</div>}
            </div>
            <div style={card}>
      ...
      {showReceipt && <InvoiceView txn={showReceipt} settings={settings} onClose={()=>{ setShowReceipt(null); setTab(showReceipt?.cancelled?"history":"billing"); }} />}
      {editTxn && <EditInvoiceModal txn={editTxn} products={products} settings={settings} onSave={handleEditSave} onCancel={()=>setEditTxn(null)} onCancelInvoice={()=>handleCancelInvoice(editTxn)} />}
    </div>
  );
}
